...
- Tagging `vX.Y.Z` triggers `.github/workflows/release.yml` to publish `dist/management.html`. In forks, plain upstream-style tags such as `v1.15.1` are blocked to avoid ambiguous releases.
- The UI version shown on the System page is injected at build time (env `VERSION`, git tag, or `package.json` fallback). On fork builds, the app automatically appends a fork marker such as `+fork.lyleliu666` to avoid looking identical to upstream.
- For fork releases, do not reuse the upstream-style plain tag by itself. Prefer a fork-specific tag such as `v1.15.1-lyleliu666.1`.
