...
- 打 `vX.Y.Z` 标签会触发 `.github/workflows/release.yml`，发布 `dist/management.html`。在 fork 仓库中，像 `v1.15.1` 这种纯上游风格标签会被拦住，避免版本混淆。
- 系统信息页显示的 UI 版本在构建期注入（优先使用环境变量 `VERSION`，否则使用 git tag / `package.json`）。如果这是 fork 构建，页面会自动附加类似 `+fork.lyleliu666` 的 fork 标识，避免看起来和 upstream 完全一样。
- 对 fork 发版时，不要直接复用上游那种纯 `vX.Y.Z` 标签。建议使用类似 `v1.15.1-lyleliu666.1` 的 fork 专属版本号。
